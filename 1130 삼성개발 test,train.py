...
impact = pd.read_csv('삼성 개발 데이터셋/1.2 xsens데이터csv/impact.csv')
        ...
        for m in range(1, 15):
            for trial in range(1,4):
                try:
                    ...
                    path = "삼성 개발 데이터셋/1.2 xsens데이터csv/csv/"
                    name = "S%s%s%sR%s" % (e, movement, m, t)
                    ...
                    if movement == 'F':
                        ...
                        for i in range(len(impact)):
                            if name == impact.name[i]:
                                impact_frame = impact.impact[i]    #F니까 impact_frame 기준
                                break
                        Window = data.iloc[impact_frame - 120:impact_frame + 120, :]  #120을 행으로 잡음

                    else:  #D라면
                        # 중간 지점 찾기 ADL
                        median_frame = np.argsort(data.index)[len(data) // 2]           # median_frame 기준
                        Window = data.iloc[median_frame - 120:median_frame + 120, :]
            ...
            for trial in range(1,4):
                try:
                    ...
                    if movement == 'F':
                        ...
                        for i in range(len(impact)):
                            if name == impact.name[i]:
                                impact_frame = impact.impact[i]
                                break
                        Window = data.iloc[impact_frame - 120:impact_frame + 120, :]
                    else:
                        # 중간 지점 찾기 ADL
...
#모델만들기위해
X_train = train_data.iloc[:, :960]
Y_train = train_data.FD

X_test = test_data.iloc[:, :960]
Y_test = test_data.FD
